function SwaggerService(discoveryUrl, _apiKey, statusCallback) {
  ...
  if (apiKey) {
    ...
    if (apiKey.length > 0)
    apiKeySuffix = "?sessionToken=" + apiKey;
  }
...
api.setBaseUrl(basePath);
 ...
 init: function(atts) {
     ...
     var trimmed = this.path.replace(/^\s+|\s+$/g, ''); //removes whitespace
     this.name = trimmed.replace(".{format}", "").replace(/\//g, "_").replace(/[{}]/g, '');    
...
o.baseUrl = u;
  ...
  ApiOperation.include({
    ...
    invocationUrl: function(formValues) {
      ...
      var apipath = new Array();
      var pathstring;
      var pathstr;
      apipath = urlTemplateText.split("$");
          for(var i=0;i<apipath.length;i++){
              if(apipath.indexOf("$") === -1){
       pathstr=apipath[i];
                pathstring=pathstr.substring(pathstr.indexOf("{")+1,pathstr.indexOf("}"));
       if(pathstring){
         apipath[i]=pathstring;
       }
              }
          }
      ...
      this.parameters.each(function(param) {
        ...
        if (param.paramType == "path" && paramValue.length > 0) {
        if($.inArray(param.name, apipath) > -1) {}
            else {
      ...
      var apiurl = this.baseUrl.replace("/apidoc","");  
...
urlsTried = urlsTried + "<br/>" + this.discoveryUrlList[i];
   ...
   $.ajax ({	 
            ...
            jsonpCallback: null,
            success: function(response)	{
       console.log("response in fetchResource:" +response);
   	   console.log("apiResource in fetchResource:" +apiResource);

...
public class ServicePartyAssocRepositoryHibernate extends BaseRepositoryHibernate implements ServicePartyAssocRepository  {
	...
	public Map<String, Object> getPartyVersion(String userUid, String organizationUid,String groupUid) {
		Session session = getSession();
		String sql = "select s.version, case spa.party_type when 'user' then 1 when 'group' then 2 " + " when 'organization' then 3 else 4 end type_order from service_party_assoc  spa " + " left outer join user u on u.gooru_uid = spa.party_uid and spa.party_type = 'user' "
				+ " left outer join party ug on ug.party_uid = spa.party_uid and spa.party_type = 'group' " + " inner join service s on s.service_key = spa.service_key " + " where spa.party_uid = '" + userUid + "' or spa.party_uid = '" + organizationUid + "' ";
		...
		return iteratePartyVersion(arrayList(query));
	}
